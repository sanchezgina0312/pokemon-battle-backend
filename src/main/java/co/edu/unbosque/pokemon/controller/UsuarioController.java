 ...
 * autenticación básica, traducción de texto y actualización de atributos del
 * usuario autenticado.
 */
@RestController
...
public class UsuarioController {
	...
	public ResponseEntity<String> actualizarUsuario(@RequestBody UsuarioDTO usuarioDTO) {
		try {
			...
			return (status == 0) ? new ResponseEntity<>("Usuario actualizado exitosamente", HttpStatus.ACCEPTED)
	 ...
	 */
	@PutMapping("/genero")
	public ResponseEntity<String> actualizarGenero(@RequestParam String genero, Authentication authentication) {
		...
		return (status == 0) ? new ResponseEntity<>("Personaje guardado exitosamente", HttpStatus.ACCEPTED)
